...
async function crear(req, res) {
  try {
    ...
    getIO(req).emit('movimiento:creado', nuevoMovimiento);
...
async function actualizar(req, res) {
  try {
    ...
    const [existe] = await pool.query('SELECT id FROM movimientos WHERE id = ?', [id]);
    if (existe.length === 0) {
      return res.status(404).json({ ok: false, mensaje: 'Movimiento no encontrado' });
    }
    ...
    await pool.query(
      `UPDATE movimientos SET
        ...
        fecha = ?, tipo_registro = ?, estado = ?, descripcion = ?
       WHERE id = ?`,
      [categoria_id, concepto, tipo_movimiento, monto, fecha, tipo_registro, estado, descripcion || null, id]
    );
    ...
    getIO(req).emit('movimiento:actualizado', actualizado);
...
async function cambiarEstado(req, res) {
  try {
    ...
    await pool.query('UPDATE movimientos SET estado = ? WHERE id = ?', [estado, id]);
    ...
    const actualizado = rows[0];
    getIO(req).emit('movimiento:estado-cambiado', actualizado);
...
async function eliminar(req, res) {
  try {
    ...
    getIO(req).emit('movimiento:eliminado', { id: Number(id) });
...
async function resumenDashboard(req, res) {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT tipo_movimiento, tipo_registro, estado, monto
       FROM movimientos WHERE anio = ? AND mes = ?`,
      [anio, mes]
    );
